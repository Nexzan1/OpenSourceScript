-- Nexzan Hub • Swing For Eggs • reference UI + local HWID key system v3.9.3
...
local function inferThemeRole(instance, property)
    ...
    local base={
        ...
        Accent=Color3.fromRGB(178,109,255), Glow=Color3.fromRGB(223,200,255),
        Text=Color3.fromRGB(249,242,255), TextDim=Color3.fromRGB(187,164,210),
        Danger=Color3.fromRGB(255,124,160),
    }
    for role,color in pairs(base) do
        if value==color or value==UI.Theme[role] then return role end
    end
    local name=tostring(instance.Name):lower()
    local text=instance:IsA('TextLabel') or instance:IsA('TextButton') or instance:IsA('TextBox')
    if property=='BackgroundColor3' then
        if name:find('background',1,true) or name=='root' then return 'Background' end
        if name:find('surface',1,true) or name:find('panel',1,true) or name:find('card',1,true) or name:find('section',1,true) or name:find('container',1,true) then return 'Surface' end
        if name:find('toggle',1,true) or name:find('slider',1,true) or name:find('active',1,true) then return 'Accent' end
        local lum=(value.R+value.G+value.B)/3
        if lum<0.16 then return 'Background' end
        if lum<0.36 then return 'Surface' end
    elseif property=='TextColor3' or property=='PlaceholderColor3' then
        if name:find('description',1,true) or name:find('subtitle',1,true) or name:find('hint',1,true) or name:find('status',1,true) or name:find('dim',1,true) then return 'TextDim' end
        if text then
            local lum=(value.R+value.G+value.B)/3
            return lum<0.72 and 'TextDim' or 'Text'
        end
    elseif property=='ImageColor3' then
        if name:find('danger',1,true) or name:find('error',1,true) then return 'Danger' end
        if name:find('icon',1,true) or name:find('leading',1,true) then return 'Glow' end
        if value ~= Color3.new(1,1,1) then return 'Glow' end
    elseif property=='Color' then
        if name:find('border',1,true) or name:find('stroke',1,true) then return 'Accent' end
    end
end
local function bindInferredThemeRole(instance, property)
    local role=inferThemeRole(instance,property)
    if role then bindColor(instance,property,role) end
end
...
local function style(instance)
    ...
    if not pickerPopup and not gradientEntries[instance] then
        ...
        if property then
            ...
            local matched=false
            for _,role in ipairs({"Background","Surface","Accent","Glow","Text","TextDim","Danger"}) do
                if color == UI.Theme[role] then bindColor(instance,property,role); matched=true; break end
            end
            if not matched then bindInferredThemeRole(instance,property) end
        end
        if instance:IsA("TextLabel") or instance:IsA("TextButton") or instance:IsA("TextBox") then
            bindInferredThemeRole(instance,"TextColor3")
            if instance:IsA("TextBox") then bindInferredThemeRole(instance,"PlaceholderColor3") end
        elseif instance:IsA("ImageLabel") or instance:IsA("ImageButton") then
            bindInferredThemeRole(instance,"ImageColor3")
        end
    end
end
...
local Themes = (function()
    ...
    function api.Apply(palette, name)
        ...
        for _, instance in ipairs(main:GetDescendants()) do
            if instance:IsA("UIGradient") and not instance:FindFirstAncestor("ColorPickerPopup") and not gradientEntries[instance.Parent] then
                instance.Color = ColorSequence.new(palette.Accent, palette.Glow)
            end
            if instance.Parent then style(instance) end
        end
        -- Hard refresh every visible themeable class. Selection-specific colors are applied last below.
        for _, instance in ipairs(main:GetDescendants()) do
            if not instance:FindFirstAncestor("ColorPickerPopup") and not instance:GetAttribute("NexzanColorWell") then
                if instance:IsA("TextLabel") or instance:IsA("TextButton") then
                    local lower=tostring(instance.Name):lower()
                    if lower:find("danger",1,true) or lower:find("error",1,true) or lower:find("delete",1,true) then
                        instance.TextColor3=palette.Danger
                    elseif instance:IsA("TextLabel") and (instance.Name=="Description" or instance.FontFace==UI.Theme.FontRegular) then
                        instance.TextColor3=palette.TextDim
                    else
                        instance.TextColor3=palette.Text
                    end
                elseif instance:IsA("ImageLabel") or instance:IsA("ImageButton") then
                    local lower=tostring(instance.Name):lower()
                    if not lower:find("profile",1,true) and not lower:find("headshot",1,true) and not lower:find("background",1,true) then
                        if lower:find("danger",1,true) or lower:find("error",1,true) then instance.ImageColor3=palette.Danger
                        elseif lower:find("icon",1,true) or lower:find("leading",1,true) or lower=="imagelabel" then instance.ImageColor3=palette.Glow end
                    end
                elseif instance:IsA("UIStroke") then
                    instance.Color=palette.Accent
                ...
                elseif instance:IsA("Frame") or instance:IsA("CanvasGroup") then
                    if instance.BackgroundTransparency<1 then
                        ...
                        if color==Color3.new(1,1,1) or (color.R+color.G+color.B)/3<0.36 then instance.BackgroundColor3=palette.Surface end
                    end
                end
            end
        end
        -- Tabs, subtabs, feature icons, feature titles, and descriptions use explicit final roles.
        if Window._tabIndicator then Window._tabIndicator.BackgroundColor3 = palette.Accent end
        for _, tab in ipairs(Window._tabs or {}) do
            local selected = Window._currentTab == tab
            if tab._label then tab._label.TextColor3 = selected and palette.Text or palette.TextDim end
            if tab._icon then
                if gradientEntries[tab._icon] then gradientEntries[tab._icon] = nil end
                for _, child in ipairs(tab._icon:GetChildren()) do if child:IsA("UIGradient") then child.Enabled = false end end
                tab._icon.ImageColor3 = selected and palette.Text or palette.TextDim
            end
            if tab._subTabIndicator then tab._subTabIndicator.BackgroundColor3 = palette.Accent end
            for _, sub in pairs(tab._subTabs or {}) do
                local subSelected = sub.Selected == true
                if sub.Label then sub.Label.TextColor3 = subSelected and palette.Text or palette.TextDim end
                if sub.Icon then sub.Icon.ImageColor3 = subSelected and palette.Text or palette.TextDim end
            end
        end
